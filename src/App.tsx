...
import { Project, EditionMode } from './types';
...
export default function App() {
  ...
  const [editionMode, setEditionMode] = useState<EditionMode>(() => {
    const saved = localStorage.getItem('karrents_edition');
    return (saved === 'pro' || saved === 'community') ? (saved as EditionMode) : 'community';
  });

  const [isUpgradeModalOpen, setIsUpgradeModalOpen] = useState<boolean>(false);

  const handleActivatePro = (licenseKey?: string) => {
    setEditionMode('pro');
    localStorage.setItem('karrents_edition', 'pro');
    setUserPlan('Pro Edition (Commercial)');
    localStorage.setItem('karrents_plan', 'Pro Edition (Commercial)');
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans select-none antialiased">
        ...
        <div className="flex-grow flex h-screen overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-zinc-950 overflow-hidden">
            ...
            <header className="h-16 border-b border-zinc-800/50 bg-zinc-950 px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
              ...
              {/* System metrics & Edition Badging */}
              <div className="hidden sm:flex items-center gap-3 font-mono text-[10px]">
                {/* Edition Tier Badge */}
                {editionMode === 'community' ? (
                  <button
                    onClick={() => setIsUpgradeModalOpen(true)}
                    className="flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 hover:bg-amber-500/20 text-[10px] font-mono font-bold px-2.5 py-1 rounded-full transition-all cursor-pointer shadow-sm"
                  >
                    <span>COMMUNITY EDITION (FREE)</span>
                    <span className="bg-amber-500 text-zinc-950 text-[9px] px-1.5 py-0.2 rounded font-black uppercase">UPGRADE TO PRO</span>
                  </button>
                ) : (
                  <button
                    onClick={() => setIsUpgradeModalOpen(true)}
                    className="flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 text-[10px] font-mono font-bold px-2.5 py-1 rounded-full transition-all cursor-pointer"
                  >
                    <Sparkles className="w-3 h-3 text-emerald-400" />
                    <span>PRO EDITION (LICENSED)</span>
                  </button>
                )}
                ...
                <button
                  ...
                  onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
                  className="p-1.5 rounded-md hover:bg-zinc-800/40 text-zinc-400 hover:text-white transition-colors border border-zinc-800/50 flex items-center justify-center cursor-pointer ml-1"
                  title={`Switch to ${theme === 'dark' ? 'Light' : 'Dark'} Mode`}
                >
      ...
      {/* Global Upgrade to Pro Modal Overlay */}
      <UpgradeModal
