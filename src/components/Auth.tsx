...
export default function Auth({ onLoginSuccess, userEmail, onClose }: AuthProps) {
  ...
  // Real user persistence using localStorage
  const [registeredUsers, setRegisteredUsers] = useState<Record<string, string>>(() => {
    const saved = localStorage.getItem('karrents_users');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('karrents_users', JSON.stringify(registeredUsers));
  }, [registeredUsers]);
  ...
  const handleGoogleAccountSelected = (email: string) => {
      ...
      <main className="flex-grow flex items-center justify-center p-4 relative z-10">
        <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-12 gap-8 bg-zinc-900/40 border border-zinc-850 p-6 md:p-8 rounded-2xl shadow-2xl shadow-black/60 backdrop-blur-lg">
          ...
          <div className="md:col-span-6 flex flex-col justify-center space-y-6">
            ...
            {/* Step 1.5: Google Account Chooser */}
              ...
              return (
                <div className="space-y-5 animate-fade-in text-zinc-100">
                  <div className="text-center space-y-1.5 pb-2">
                    ...
                    <h3 className="text-sm font-bold font-sans text-white">Sign in with Google</h3>
                    <p className="text-[11px] text-zinc-400">Choose an account to continue to Karrents Security Intelligence</p>
                  </div>

                  <div className="space-y-2 max-h-[180px] overflow-y-auto custom-scrollbar pr-1">
                    {/* Account 1: engr.buru@gmail.com */}
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <div className="shrink min-w-0 flex-grow">
                        <span className="text-[11px] font-bold text-zinc-200 block group-hover:text-white leading-none">engr.buru@gmail.com</span>
                  ...
                  <div className="border-t border-zinc-850/60 pt-3.5 space-y-2">
                    ...
                    <form onSubmit={(e) => {
                      ...
                      if (customEmail.trim() && customEmail.includes('@')) {
                        handleGoogleAccountSelected(customEmail.trim().toLowerCase());
                      }
                  ...
                  <div className="flex gap-2.5 pt-1">
                    <button
                      ...
                      className="w-full bg-zinc-950 hover:bg-zinc-900 border border-zinc-850 text-zinc-400 hover:text-white px-3.5 py-2 rounded-lg text-xs font-semibold transition-colors text-center cursor-pointer"
                    >
                      Cancel / Back
                    </button>
                  </div>
